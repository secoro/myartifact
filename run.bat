...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f myartifact-share
	call %MVN_EXEC% clean package -pl myartifact-share,myartifact-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill myartifact-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f myartifact-acs
	call %MVN_EXEC% clean package -pl myartifact-integration-tests,myartifact-platform,myartifact-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl myartifact-platform,myartifact-integration-tests,myartifact-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl myartifact-platform,myartifact-integration-tests
EXIT /B 0
:purge
